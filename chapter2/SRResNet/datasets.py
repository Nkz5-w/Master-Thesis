...
class SRDataset(Dataset):
    def __init__(self, data_folder, split, crop_size, scaling_factor, lr_img_type, hr_img_type, test_data_name=None):
        ...
        if self.split == 'train':
            assert self.crop_size % self.scaling_factor == 0, ("Crop dimensions are not perfectly divisible by scaling "
                                                               "factor! This will lead to a mismatch in the "
                                                               "dimensions of the original HR patches and their "
                                                               "super-resolved (SR) versions!")

        # Read list of image-paths
        if self.split == 'train':
            with open(os.path.join(data_folder, 'train_images_ckm_pathloss.json'), 'r') as j:
                self.images = json.load(j)
        else:
            with open(os.path.join(data_folder, 'test_images_ckm_pathloss.json'), 'r') as j:
                self.images = json.load(j)
