...
class SRResNet(nn.Module):
    def __init__(self, large_kernel_size=9, small_kernel_size=3, n_channels=64, n_blocks=16, scaling_factor=4):
        ...
        assert scaling_factor in {2, 4, 8, 16}, "The scaling factor must be 2, 4, 8 or 16!"
...
if __name__ == '__main__':
    # 统计参数量
    srresnet = SRResNet()
    total_params = sum(p.numel() for p in srresnet.parameters())
    srgan_g = Generator()
    total_params_1 = sum(p.numel() for p in srgan_g.parameters())
    srgan_d = Discriminator()
    total_params_2 = sum(p.numel() for p in srgan_d.parameters())
    print(total_params)
    print('\n')
    print(total_params_1)
    print('\n')
    print(total_params_2)
    print('\n')
    print(total_params_2+total_params_1)
