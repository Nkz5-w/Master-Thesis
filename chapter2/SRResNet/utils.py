...
def create_data_lists(train_folders, test_folders, min_size, output_folder):
    ...
    with open(os.path.join(output_folder, 'train_images_ckm_pathloss.json'), 'w') as j:
    ...
    for d in test_folders:
        ...
        with open(os.path.join(output_folder, 'test_images_ckm_pathloss.json'), 'w') as j:
            json.dump(test_images, j)

    print(
        "\nData lists created successfully!\nJSONS containing lists of Train and Test images have been saved to %s\n" % output_folder)
...
class ImageTransforms(object):
    ...
    def __call__(self, img):
        ...
        if self.split == 'train':
            ...
            # Take the largest possible center-crop
            x_remainder = img.width % self.scaling_factor
            y_remainder = img.height % self.scaling_factor
            left = x_remainder // 2
            top = y_remainder // 2
